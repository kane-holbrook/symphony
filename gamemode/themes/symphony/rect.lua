...
-- Border: draw a mesh as a ribbon
-- Poly->Mesh
-- RoundedBox
-- Why do custom props need a relayout?
-- Refactor
-- Optimize
...
local function BaseExtent(panel, prop, value, useHeight)
    ...
    elseif string.EndsWith(value, "pw") then
        ...
        local f = wrapfunc(function() 
            ...
            return math.Round(percent * (Parent.Width - Parent:GetPaddingLeft() - Parent:GetPaddingRight()), 0)
        end)
        f.Value = percent
        f.Mode = "pw"
    ...
    elseif string.EndsWith(value, "ph") then
        local percent = tonumber(string.sub(value, 1, -3))
        local f = wrapfunc(function() return math.Round(percent * (Parent.Height - Parent:GetPaddingTop() - Parent:GetPaddingBottom()), 0) end)
        f.Value = percent
        f.Mode = "ph"
        panel:SetPropertyComputed(prop, f)
        return nil
...
Theme.Default.Controls["Rect"] = Rect

Rect.Theme = Theme.Default

Rect:CreateProperty("Panel")
Rect:CreateProperty("Theme", nil, { Manual = true })
...
Rect:CreateProperty("Page", Type.String)
...
Rect:CreateProperty("Blur", Type.Number, { Default = 0 })
Rect:CreateProperty("ColorMod", Type.Table)
...
function Rect.Prototype:GetTheme()
...
local coltex = Material("pp/colour")

function Rect.Prototype:Paint(w, h, p)

    local poly = self:GetShape() and self.Poly
    

    local colmod = self:GetColorMod()
    if colmod then
        local x, y = p:ScreenToLocal(0, 0)

        surface.SetDrawColor(255, 255, 255)
        surface.SetMaterial(coltex)
        for k, v in pairs(colmod) do
            coltex:SetFloat("$" .. k, v)
        end
...
function Rect.Prototype:RenderProperty(name, force)
    ...
    if IsValid(panel) then
        ...
        local setter = opt and opt.Options.Set
        if setter then
            local getter = opt and opt.Options.Get
            if getter then
...
function Rect.Prototype:CalculateBounds(force)
    ...
    if align then
        ...
        if growElement then
            if ltr then
                local sz = math.Round(w - tw - pl - pr - (gap * (numNonGrowElements - 1)), 0)
...
WorldPanel:SetTheme(Theme.Symphony)
