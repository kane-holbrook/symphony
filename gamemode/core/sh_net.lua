...
-- Based on https://raw.githubusercontent.com/Facepunch/garrysmod/refs/heads/master/garrysmod/lua/includes/extensions/net.lua
...
net.Receivers = {}
net.Listeners = weaktable(false, true)

function net.Listen(name, ttl)
    local p = Promise.Create()
    p:SetTTL(ttl or 30)
    net.Listeners[name] = p
    return p
end
...
function net.WriteColor(col, writeAlpha)
    ...
    net.WriteUInt(g, 8)
    net.WriteUInt(b, 8)
    if writeAlpha then net.WriteUInt(a, 8) end
end
