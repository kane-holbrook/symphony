...
function LABEL:ParseNodeChild(el, child, ctx)
    if child.Tag == "Inline" then
        self:SetText(child.Attributes.Text)
        return
    end
    
    Rect.ParseNodeChild(self, el, child, ctx)
end
...
function LABEL.Prototype:ReceiveEvent(event, ...)
    ...
    if event == "Change:Text" or event == "Change:Font" or event == "Parent:Change:Font" or event == "Parent:Layout" then
        self:SizeToChildren()
    end
end
