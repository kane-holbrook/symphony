...
function BasePanel:LoadXML()
    ...
    for k, v in pairs(t.Children) do
        Interface.CreateFromNode(self, v, { Scope = self, IgnoreSlots = true })
    end
end
...
do
    ...
    function BasePanel:OnPropertyChanged(name, value, old)
        if name == "Ref" then
            ...
            while p do
                tgt = p
                if tgt:GetProperty("Ref", true) or tgt.Scope then
                    break
                end

                local p2 = p:GetParent()
                if p2 == vgui.GetWorldPanel() then
                    break
                end
        ...
        if self.Properties then
            ...
            for k, p in pairs(self.Properties) do                    
                ...
                if p.Func then
                    ...
                    local num, values = GetPropertyResult(pcall(p.Func, self)) 
                    if not num then
                        ErrorNoHaltWithStack("Error in property " .. k .. ": " .. values)
                        continue
                    end                    
