...
function parser:text(text)
    local el = { name = "Text", attrs = { Content = string.Trim(text) } }
    self:starttag(el)
    self:endtag(el)
end
...
function Rect:CreateFromNode(parent, node, obj)
    obj = obj or Interface.Create(self, parent)
...
function Rect.Prototype:ParseChildNode(child)
    local typ = Type.GetByName(child.Tag)
...
function Interface.CreateFromXML(parent, xml)
    ...
    return Interface.CreateFromNode(parent, t)
end

function Interface.RegisterFromXML(name, xml, options)
    ...
    local base = Type.GetByName(d.Tag)
    assert(base)

    local t = Interface.Register(name, base, options)
    t.Xml = d
    return t
end

function Interface.CreateFromNode(parent, t)
    local rtn = {}
    for k, v in pairs(t.Children) do
        local typ = Type.GetByName(v.Tag)
...
-- Was working on SHAPES. I think the Poly is built correctly now, need to draw.