...
local function parseTag(self, s)
    ...
    local parseFunction = function (k, v) 
            table.insert(tag.attrs, { Name = k, Value = parseEntities(self, v) })
            tag.attrs._ = 1 
          end
...
local function parseNormalTag(self, xml, f)
    ...
    local cp = vgui.GetControlTable(tag.name)

    if cp and cp.OverrideXML and f.endt1 ~= "/" then
        local escapedTagName = string.gsub(tag.name, "%.", "%%.")
        local closingPattern = "</%s*" .. escapedTagName .. "%s*>"
    
        local dhtmlEndStart, dhtmlEndFinish = string.find(xml, closingPattern, f.endMatch + 1)
        if dhtmlEndStart then
            -- handle opening tag as normal:
            table.insert(self._stack, tag.name)
            if fexists(self.handler, 'starttag') then
                self.handler:starttag(tag, f.match, f.endMatch)
            end
    
            -- treat everything in between as text:
            local dhtmlContent = string.sub(xml, f.endMatch + 1, dhtmlEndStart - 1)
            if fexists(self.handler, 'text') then
                self.handler:text(dhtmlContent, nil, f.endMatch + 1, dhtmlEndStart - 1)
            end
    
            -- handle closing:
            if fexists(self.handler, 'endtag') then
                self.handler:endtag(tag, dhtmlEndStart, dhtmlEndFinish)
            end
    
            -- pop from stack
            table.remove(self._stack)
    
            -- tell the main loop to advance:
            f.endMatch = dhtmlEndFinish
            return tag
        else
            err(self, "Unterminated <" .. tag.name .. "> section", f.pos)
            return tag
        end
    end
