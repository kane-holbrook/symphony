...
function dragndrop.StartDragging()
	...
	for k, v in pairs( dragndrop.m_Dragging ) do
		...
		local cx, cy = v:CursorPos()
		-- cx, cy are from top left, but I need the offset from the centre...
		v.DragDropOffset = Vector(v:GetWide() / 2 - cx, v:GetTall() / 2 - cy, 0)
...
function dragndrop.StopDragging()
	...
	for k, v in pairs( dragndrop.m_Dragging or {} ) do
		...
		if ( !IsValid( v ) ) then continue end
		v:OnStopDragging(v.DragDropOffset)
		v.DragDropOffset = nil
	...
	local wasEnabled = DisableClipping( true )
		...
		surface.SetAlphaMultiplier( Alpha )
			...
			for k, v in pairs( dragndrop.m_Dragging ) do
				...
				v:PaintAt( ox + v.x + v.DragDropOffset.x - v:GetWide() / 2, oy + v.y + v.DragDropOffset.y - v:GetTall() / 2 ) -- fill the gap between the top left corner and the mouse position
